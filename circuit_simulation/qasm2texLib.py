...
#
# Outputs: latex qasm_file (to stdout)
#
...
# -----------------------------------------------------------------------------
#
# Edited on 01-June-15 by E. R. Schmidgall to function as a python library instead of
# python script and changed for compatibility with Python 3 (not tested though).
#
...
#
# This qasm_file, qasm2tex, is part of qasm2circ
#
...
def num2name(num):  # convert a number to a name
    if (num == 0):
        ...
        return chr((num % 26) + 64) + num2name(int(num / 26))
...
class qasm_parser:  # parser for qasm; inputs lines, returns
    ...
    def __init__(self, fp):
        ...
        self.comments = ''  # string with comments from original qasm qasm_file
...
class qcircuit:  # quantum circuit class
    ...
    def output_sequence(self, fileName):  # output time-sequence of gates

        try:
            f = open(fileName, 'w')
        except IOError:
            print(fileName + " may not exist. Verify.")
            return
        ...
        for timestep in self.circuit:  # loop over timesteps
            ...
            for g in timestep:  # loop over events in this timestep
                op = self.optab[g]
                f.write("%%    Gate %02d %s(%s) \n" % (op.id,
                                                       op.name, op.args))
            k += 1
        f.write("\n")
        f.close()

    def output_matrix(self, fileName):  # output circuit matrix, of qubit vs timestep
        ...
        k = 0
        f.write("% Qubit circuit matrix:\n%")
        for y in self.matrix:  # loop over qubits
            toPrint = self.qubitnames[k]
            toPrint2 = ",".join(y)
            # f.write('%% %s: %s \n' % (self.qubitnames[k],join(y,', ')))
            f.write('%% %s: %s \n' % (toPrint, toPrint2))
            k += 1
    ...
    def output_latex(self, fileName):  # output latex with xypic for circuit
        ...
        f.write("\n")
        f.write("% definitions for the circuit elements \n")
        for g in self.optab:
            strNew = g.latex()
            strNew = strNew + '\n'
            f.write(strNew)  # output \def\gXY{foo} lines

        # now output defs for qubit labels and initial states
        f.write("\n")
        f.write("% definitions for bit labels and initial states \n")

        for j in range(len(self.matrix)):
            qb = self.qubitnames[j]
            f.write(r"\def\b%s{%s}" % (num2name(j + 1), self.qb2label(qb)) + "\n")

        # now output circuit
        f.write("\n")
        # print r'\xymatrix@R=15pt@C=12pt{'
        f.write("% The quantum circuit as an xymatrix \n")
        f.write(r"\xymatrix@R=5pt@C=10pt{ " + "\n")
        ...
        f.write("% \n")
        f.write("% Vertical lines and other post-xymatrix latex % \n")
        for g in self.optab:
            if (g.endtex != ""):
                strTmp = g.endtex
                f.write(strTmp)  # output end latex commands
...
def main(qasm_file, pdf_file_name):
    qp = qasm_parser(qasm_file)  # parse the qasm qasm_file
    qc = qcircuit(qp.nametab, qp.typetab)  # initialize the circuit
    for g in qp.gatetab:  # add each gate to the circuit
        qc.add_op(g)

    qc.output_latex(pdf_file_name)  # output latex code
