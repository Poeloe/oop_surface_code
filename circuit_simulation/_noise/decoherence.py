...
def N_decoherence_new(self, qubits):
    for qubit in qubits:
        ...
        if waiting_time_idle > 0:
            ...
            self._N_phase_damping_channel(rel_qubit, density_matrix, rel_num_qubits, waiting_time_idle, T2_idle)
        ...
        if waiting_time_lde > 0:
            ...
            self._N_phase_damping_channel(rel_qubit, density_matrix, rel_num_qubits, waiting_time_idle, T2_idle)
            if qubit_type == 'n':
                self._N_phase_damping_channel(rel_qubit, density_matrix, rel_num_qubits, waiting_time_lde, T2_lde)
                self._N_amplitude_damping_channel(rel_qubit, density_matrix, rel_num_qubits, waiting_time_lde, T1_lde)
