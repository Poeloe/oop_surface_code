...
def cluster_place_bucket(graph, cluster):
    ...
    #     else:
    #         cluster.bucket = None
    cluster.bucket = cluster.size - cluster.full_edged
...
def find_clusters(graph, anyon_order="random", uf_plot=None, plot_step=False):
    ...
    method = "C"
    if method in ["A", "B"]:
        graph.numbuckets = graph.size - graph.size % 2
        graph.buckmax = [(i+1)**2 + (i+2)**2 for i in range(graph.size//2)]
    elif method == "C":
...
def grow_clusters(graph, uf_plot=None, plot_step=False, print_steps=True, intervention=False):
    ...
    '''
    def grow(graph, cluster, root_cluster, full_edged, uf_plot, plot_step, print_steps, intervention, family_growth=True):
        '''
        ...
        while cluster.foster != []:
            ...
            grow(graph, foster_cluster, root_cluster, full_edged, uf_plot, plot_step, print_steps, intervention, False)
        ...
        if family_growth:
            ...
            while cluster.childs != []:                                 # First go through child clusters
                ...
                grow(graph, child_cluster, root_cluster, full_edged, uf_plot, plot_step, print_steps, intervention)
    ...
    for bucket_i, bucket in enumerate(graph.buckets):
        ...
        while bucket != []:                          # Loop over all clusters in the current bucket
            ...
            if root_cluster.bucket == bucket_i:                  # Check that cluster is not already in a higher bucket
                ...
                grow(graph, root_cluster, root_cluster, root_cluster.full_edged, uf_plot, plot_step, print_steps, intervention)
